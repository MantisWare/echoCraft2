...
!define SHERPA_FIREWALL_RULE "EchoCraft Local Transcription Server (sherpa-onnx)"
...
!macro customUnInstall
  ...
  ${ifNot} ${isUpdated}
    !insertmacro DeleteAutoStartEntry "com.mantisware.echocraft"
    !insertmacro DeleteAutoStartEntry "com.mantisware.echocraft.staging"
    !insertmacro DeleteAutoStartEntry "com.mantisware.echocraft.development"
    nsExec::ExecToLog 'netsh advfirewall firewall delete rule name="${SHERPA_FIREWALL_RULE}"'
    ; Downloaded models live in the shared .cache\openwhispr tree that a
    ; parallel OpenWhispr install may still use, so only the EchoCraft-owned
    ; cache is removed here.
    StrCpy $0 "$PROFILE\.cache\echocraft"
    IfFileExists "$0\*.*" 0 +3
      RMDir /r "$0"
      DetailPrint "Removed EchoCraft cache"
  ${endIf}
!macroend
