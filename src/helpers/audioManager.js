  ...
  async processAudio(audioBlob, metadata = {}, processingPipeline = null) {
    ...
    try {
      ...
      if (useLocalWhisper) {
        ...
        if (!isSignedIn) {
          const err = new Error(
            "EchoCraft Cloud requires sign-in. Please sign in again or switch to BYOK mode."
          );
  ...
  async startStreamingRecording(forceDefaultMic = false) {
    ...
    try {
      ...
      if (error.name === "NotAllowedError" || error.name === "PermissionDeniedError") {
        ...
        errorDescription =
          "Your EchoCraft Cloud session is unavailable. Please sign in again from Settings.";
