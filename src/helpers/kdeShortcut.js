...
const COMPONENT_NAME = "echocraft";
...
class KDEShortcutManager {
  ...
  _findSlotByFriendlyName(name) {
    ...
    for (const slotName of this.registeredSlots) {
      friendlyToSlot[`EchoCraft ${slotName}`] = slotName;
      friendlyToSlot[`EchoCraft`] = "dictation"; // legacy compat
    }
  ...
  async registerKeybinding(electronHotkey, slotName = "dictation", callback, isPushToTalk = false) {
    ...
    const actionId = [COMPONENT_NAME, slotName, "EchoCraft", `EchoCraft ${slotName}`];
