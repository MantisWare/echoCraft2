...
class GoogleCalendarOAuth {
  ...
  startOAuthFlow() {
    if (!this.getClientId() || !this.getClientSecret()) {
      // Without this the browser opens on a client_id=undefined authorize URL
      // and the flow hangs for the full 120s loopback timeout. The UI hides
      // unconfigured providers; this guards every other caller.
      throw new Error("GOOGLE_CALENDAR_CLIENT_ID / _SECRET is not configured");
    }
