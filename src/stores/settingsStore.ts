...
function deriveTranscriptionMode(
  ...
  // OpenWhispr Cloud is retired: fall back to local inference.
  return "local";
}
...
function migrateProviderSettings() {
  ...
  // OpenWhispr Cloud is retired: default to local inference.
...
function migrateRetiredOpenwhisprModes() {
  if (!isBrowser) return;
  if (localStorage.getItem("_openwhisprRetiredMigrated") === "1") return;

  const modeKeys: Array<keyof SettingsState> = [
    "transcriptionMode",
  ...
  for (const key of modeKeys) {
    ...
    if (stored === "openwhispr") localStorage.setItem(String(key), "local");
  }
  ...
  for (const key of cloudModeKeys) {
    const stored = localStorage.getItem(key);
    if (stored === "openwhispr") localStorage.setItem(key, "byok");
  }

  localStorage.setItem("_openwhisprRetiredMigrated", "1");
}

migrateRetiredOpenwhisprModes();
...
const SECRET_IPC_SAVERS = {
  ...
  huggingface: "saveHuggingfaceKey",
...
export const useSettingsStore = create<SettingsState>()((set, get) => ({
  ...
  setHuggingfaceApiKey: createSecretSetter("huggingfaceApiKey", "huggingface", "huggingface"),
  ...
  updateApiKeys: (keys: Partial<ApiKeySettings>) => {
    ...
    if (keys.huggingfaceApiKey !== undefined) s.setHuggingfaceApiKey(keys.huggingfaceApiKey);
...
export function selectPolicyEffectiveSettings(
  ...
  for (const keys of TRANSCRIPTION_CONTEXT_KEYS) {
    ...
    // OpenWhispr Cloud is retired; any managed selection that requests it is
    // clamped to local so policy cannot re-enable metered cloud calls.
    const clampedMode = selection.mode === "openwhispr" ? "local" : selection.mode;

    writable[keys.mode] = clampedMode;
    writable[keys.useLocal] = clampedMode === "local";
    writable[keys.cloudMode] = "byok";

    if (clampedMode === "providers") {
  ...
  for (const scope of Object.keys(INFERENCE_SCOPES) as InferenceScope[]) {
    ...
    writable[definition.storeKeys.mode] = clampedMode;
    if (definition.storeKeys.cloudMode) {
      // OpenWhispr Cloud is retired, so every cloud scope resolves to BYOK.
      writable[definition.storeKeys.cloudMode] = "byok";
    }

    let provider = selection.provider;
    if (clampedMode === "self-hosted") provider = "lan";
    if (clampedMode === "local" && !localLlmProviderIds.has(provider)) {
    ...
    if (clampedMode === "providers" || clampedMode === "enterprise" || clampedMode === "local") {
      const providerChanged = clampedMode !== config.mode || provider !== config.provider;
      writable[definition.storeKeys.model] =
        !providerChanged && config.model
          ? config.model
          : defaultLlmModel(clampedMode, provider, state.bedrockRegion);
    }
  }
  ...
  if (window.electronAPI) {
    try {
      const [
        ...
        window.electronAPI.getHuggingfaceKey?.(),
