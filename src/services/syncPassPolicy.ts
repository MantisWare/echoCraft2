...
export function resolveSyncConsent(state: {
  ...
  return {
    shared,
    // Subscriptions have been removed: treat backup as available for all
    // signed-in users who opted in and whose org policy allows it.
    backup: shared && state.backupEnabled && state.backupAllowedByPolicy,
  };
}
