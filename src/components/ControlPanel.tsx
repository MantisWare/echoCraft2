...
export default function ControlPanel({ initialSettingsSection }: ControlPanelProps = {}) {
  ...
  useEffect(() => {
    const dispose = window.electronAPI?.onLimitReached?.(
      (_data: { wordsUsed: number; limit: number }) => {
        toast({
  ...
  useEffect(() => {
    ...
    // OpenWhispr Cloud is retired: migrate pending sessions to local inference.
  ...
  useEffect(() => {
    const cleanup = window.electronAPI?.onShowHistory?.(() => {
