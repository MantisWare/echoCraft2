...
export default function IntegrationsView(_: IntegrationsViewProps = {}) {
  const { t } = useTranslation();
  const {
    gcalAccounts,
    setGcalAccounts,
  ...
  // A provider without build-time OAuth credentials could never connect, so it
  // is not offered. Already-linked accounts keep their row regardless, or a
  // build that drops credentials would strand them with no way to disconnect.
  const calendarProviders = useCalendarProviderAvailability();
  const showGoogle = shouldShowCalendarProvider({
    configured: calendarProviders.google,
    connected: hasAccounts,
  });
  const showMicrosoft = shouldShowCalendarProvider({
    configured: calendarProviders.microsoft,
    connected: mcalAccounts.length > 0,
  });
  const showApple =
    isMac &&
    shouldShowCalendarProvider({
      configured: calendarProviders.apple,
      connected: appleCalendarConnected,
    });
  const hasNoCalendarProviders =
    calendarProviders.resolved && !showGoogle && !showMicrosoft && !showApple;
    ...
    <div className="max-w-lg mx-auto w-full px-6 py-6 space-y-5">
      ...
      <div>
        ...
        <SettingsPanel>
          ...
          {showApple && appleCalendarConnected && (
            <SettingsPanelRow>
              <div className="group flex items-center gap-3 pl-12">
