...
export default function OnboardingFlow({ onComplete }: OnboardingFlowProps) {
  ...
  const handleSetupSelection = useCallback(
    async (mode: Exclude<OnboardingSetupMode, null>, options?: { selfHosted?: boolean }) => {
      // OpenWhispr Cloud is retired, so a persisted "cloud" pick resumes on the
      // local route rather than configuring a hosted provider.
      const normalizedMode = mode === "cloud" ? "local" : mode;
