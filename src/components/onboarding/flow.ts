...
export interface OnboardingRouteContext {
  ...
  /**
   * Whether this build has account UI at all. Defaults to `ACCOUNT_UI_ENABLED`;
   * exposed so the route can be exercised both ways without a rebuild.
   */
  accountUiEnabled?: boolean;
}
...
/** First step of the flow, which is the auth step only when accounts exist. */
export const FIRST_ONBOARDING_STEP: OnboardingStepId = ACCOUNT_UI_ENABLED ? "auth" : "permissions";

export function createOnboardingSession(): OnboardingSession {
  return {
    version: ONBOARDING_FLOW_VERSION,
    currentStepId: FIRST_ONBOARDING_STEP,
    history: [],
    // With accounts hidden there is no path to choose, so every session is a
    // guest session from the start.
    authPath: ACCOUNT_UI_ENABLED ? null : "guest",
    setupMode: null,
...
function appendSetupSteps(route: OnboardingStepId[], context: OnboardingRouteContext): void {
...
export function getOnboardingRoute(context: OnboardingRouteContext): OnboardingStepId[] {
  ...
  // trigger key.
  const guestRoute: OnboardingStepId[] = [
    "auth",
    "permissions",
    "dictation-hotkey",
    "activation-mode",
  ...
  if (!(context.accountUiEnabled ?? ACCOUNT_UI_ENABLED)) {
    // No account to sign into, so there is nothing for the auth step to ask.
    const route = guestRoute.filter((stepId) => stepId !== "auth");
    appendSetupSteps(route, context);
    return route;
  }
...
export function parseOnboardingSession(value: string | null): OnboardingSession | null {
  ...
  try {
    ...
    return {
      ...
      // A session saved before accounts were hidden can still name the account
      // path; finalizeOnboarding reads this to decide whether auth was skipped.
      authPath: ACCOUNT_UI_ENABLED ? authPath : "guest",
      // OpenWhispr Cloud is retired: migrate any persisted "cloud" selection
      // to the local onboarding route.
      setupMode: setupMode === "cloud" ? "local" : setupMode,
