...
import {
  ...
  useCalendarProviderAvailability,
} from "../../hooks/useCalendarProviderAvailability";
...
export default function CalendarConnectionsStep() {
  ...
  const availability = useCalendarProviderAvailability();
  ...
  // Google and Microsoft need build-time OAuth credentials to connect at all,
  // so an unconfigured provider is dropped rather than offered — see
  // calendarProviderAvailability.js. A provider with accounts already linked
  // stays, so onboarding still reflects what is connected.
  const providers = [
    {
      id: "google" as const,
      ...
      configured: availability.google,
    },
    {
      ...
      configured: availability.microsoft,
    },
    ...(isMac
      ? [
          {
  ...
  return (
    ...
    <div className="flex w-full flex-col items-center gap-2.5">
      ...
      <div className="w-full max-w-[26rem] rounded-2xl border border-[var(--onboarding-control-border)] bg-[var(--onboarding-surface-secondary)] px-3.5 py-3">
        {/* Every provider hidden leaves the card empty; say why rather than
            render a bare box. Suppressed until availability resolves so the
            message does not flash before the answer arrives. */}
        {providers.length === 0 && availability.resolved && (
          <p className="py-1 text-center text-xs text-[var(--onboarding-text-secondary)]">
            {t("integrations.noProvidersAvailable")}
          </p>
        )}
