...
export default function UploadAudioView({ onNoteCreated, onOpenSettings }: UploadAudioViewProps) {
  ...
  if (file) {
    if (useLocalWhisper) {
      ...
      // Without accounts there is nothing to create, and the way past a
      // provider's size cap is local transcription, which the other branch
      // already offers.
      if (ACCOUNT_UI_ENABLED && byokTooLarge && !isSignedIn) {
  ...
  return (
    <div style={{ animation: "float-up 0.3s ease-out" }}>
      ...
      <div className="flex items-center gap-2 justify-center flex-wrap">
        ...
        {/* BYOK too large — signed in: switch to local for unlimited size */}
