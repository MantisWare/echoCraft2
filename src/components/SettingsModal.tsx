...
import { Sliders, Mic, Brain, UserCircle, Wrench, Keyboard, Shield, Users } from "lucide-react";
...
export default function SettingsModal({ open, onOpenChange, initialSection }: SettingsModalProps) {
  ...
  const sidebarItems: SidebarItem<SettingsSectionType>[] = useMemo(() => {
    ...
    const withoutAccountUi = ACCOUNT_UI_ENABLED
      ? items
      : items.filter((item) => item.id !== "account");
    return isSignedIn
      ? withoutAccountUi
      : withoutAccountUi.filter((item) => item.id !== "workspace");
  }, [t, isSignedIn]);

  const defaultSection: SettingsSectionType = ACCOUNT_UI_ENABLED ? "account" : "general";
  ...
  const resolveSection = (section: string | undefined): SettingsSectionType => {
    ...
    if (!ACCOUNT_UI_ENABLED && resolved === "account") return defaultSection;
    return resolved;
  };
