...
test("the command menu offers history and copy-last-transcription while recording", async (t) => {
  const markup = await renderMenu(t, { isRecording: true });
  assert.match(markup, /openHistory/);
  assert.match(markup, /copyLastTranscription/);
});
