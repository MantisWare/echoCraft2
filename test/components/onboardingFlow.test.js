...
test("guest flow keeps permissions and the hotkey before setup choice", async () => {
  ...
  assert.deepEqual(
    ...
    ["auth", "permissions", "dictation-hotkey", "activation-mode", "setup-choice"]
  );
});

test("a build without account UI drops the auth step from every route", async () => {
  const { getOnboardingRoute } = await load();
  // There is no account to sign into, so the flow opens on permissions and the
  // auth step is unreachable regardless of the path a stale session claims.
  for (const authPath of [null, "guest", "account"]) {
    assert.deepEqual(
      getOnboardingRoute({
        authPath,
        setupMode: null,
        agentAllowed: true,
        accountUiEnabled: false,
      }),
      ["permissions", "dictation-hotkey", "activation-mode", "setup-choice"]
    );
  }
...
test("required models insert a blocking step right after auth — account path only", async () => {
  ...
  const noFlag = getOnboardingRoute({
    authPath: "account",
    accountUiEnabled: true,
    setupMode: null,
    agentAllowed: true,
