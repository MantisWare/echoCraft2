...
const DBUS_COMMAND =
  "dbus-send --session --type=method_call --dest=com.mantisware.EchoCraft /com/mantisware/EchoCraft com.mantisware.EchoCraft.Toggle";
  ...
  withTempHyprConfig(async (configDir) => {
    ...
    assert.ok(
      fs
        ...
        .includes(`source = ${path.join(configDir, "echocraft-binds.conf")}\n`)
    );
    assert.match(
      fs.readFileSync(path.join(configDir, "echocraft-binds.conf"), "utf8"),
...
test(
  ...
  withTempHyprConfig(async (configDir) => {
    ...
    assert.match(fs.readFileSync(luaPath, "utf8"), /pcall\(require, .+echocraft-binds\.lua/);
    assert.equal(
      (fs.readFileSync(luaPath, "utf8").match(/echocraft-binds\.lua/g) || []).length,
      1
    );
    const binds = fs.readFileSync(path.join(configDir, "echocraft-binds.lua"), "utf8");
    assert.match(binds, /^-- EchoCraft keybinds/m);
...
test(
  ...
  withTempHyprConfig(async (configDir) => {
    ...
    assert.equal(fs.existsSync(path.join(configDir, "echocraft-binds.lua")), false);
  })
);
...
test(
  ...
  withTempHyprConfig(async (configDir) => {
    ...
    assert.equal(fs.existsSync(path.join(path.dirname(configPath), "echocraft-binds.lua")), true);
  })
);
...
test(
  ...
  withTempHyprConfig(async (configDir) => {
    ...
    assert.equal(fs.existsSync(path.join(configDir, "echocraft-binds.lua")), true);
  })
);
...
test(
  ...
  withTempHyprConfig(async (configDir) => {
    const luaPath = path.join(configDir, "hyprland.lua");
    const bindsPath = path.join(configDir, "echocraft-binds.lua");
    fs.mkdirSync(configDir, { recursive: true });
    fs.writeFileSync(
      luaPath,
      `-- lua config\ndofile("${bindsPath}")\nreturn {}\n-- trailing comment\n`
    ...
    assert.equal((content.match(/echocraft-binds\.lua/g) || []).length, 1);
...
test(
  ...
  withTempHyprConfig(async (configDir) => {
    const configPath = path.join(configDir, "hyprland.conf");
    const bindsPath = path.join(configDir, "echocraft-binds.conf");
    fs.mkdirSync(configDir, { recursive: true });
    fs.writeFileSync(configPath, "# legacy config\n");
    fs.writeFileSync(
      bindsPath,
      "# EchoCraft keybinds (managed automatically)\n" +
    ...
    assert.equal((content.match(/EchoCraft keybinds/g) || []).length, 1);
  })
);
...
test(
  ...
  withTempHyprConfig(async (configDir) => {
    ...
    const legacyBindsPath = path.join(configDir, "echocraft-binds.conf");
    fs.mkdirSync(configDir, { recursive: true });
    fs.writeFileSync(path.join(configDir, "hyprland.lua"), "-- lua config\n");
    fs.writeFileSync(confPath, "# keep this comment\nsource = ./echocraft-binds.conf\n");
    fs.writeFileSync(
      legacyBindsPath,
      "# EchoCraft keybinds (managed automatically)\n" +
