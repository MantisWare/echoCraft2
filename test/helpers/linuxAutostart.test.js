...
const ICON_THEME_SUBPATH = path.join("icons", "hicolor", "256x256", "apps", "echocraft.png");
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    process.env.APPIMAGE = "/home/user/Applications/EchoCraft.AppImage";
    assert.equal(resolveExecutablePath(), "/home/user/Applications/EchoCraft.AppImage");
  })
);
...
test(
  ...
  withTmpXdgDirs(async (root) => {
    ...
    fs.writeFileSync(path.join(installDir, "echocraft"), "#!/bin/bash\n");
    withExecPath(path.join(installDir, "echocraft-app"), () => {
      assert.equal(resolveExecutablePath(), path.join(installDir, "echocraft"));
    });
  })
);
...
test(
  ...
  withTmpXdgDirs(async (root) => {
    ...
    const orphan = path.join(root, "opt", "echocraft-app");
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    process.env.APPIMAGE = "/apps/EchoCraft.AppImage";
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    const contents = buildDesktopFileContents("/a/b/EchoCraft.AppImage", null);
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    const contents = buildDesktopFileContents('/home/o"neill/$HOME/`app`\\v/EchoCraft', null);
    assert.match(
      contents,
      /^Exec="\/home\/o\\"neill\/\\\$HOME\/\\`app\\`\\\\\\\\v\/EchoCraft" --hidden$/m
    );
  })
);
...
test(
  ...
  withTmpXdgDirs(async (root) => {
    ...
    setAutostartEnabled(true);
    assert.match(fs.readFileSync(getDesktopFilePath(), "utf8"), /^Icon=echocraft$/m);
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    process.env.APPIMAGE = "/old/path/EchoCraft.AppImage";
    setAutostartEnabled(true);

    process.env.APPIMAGE = "/new/path/EchoCraft-1.9.0.AppImage";
    assert.equal(syncAutostartEntry(), true);

    const contents = fs.readFileSync(getDesktopFilePath(), "utf8");
    assert.match(contents, /^Exec="\/new\/path\/EchoCraft-1\.9\.0\.AppImage" --hidden$/m);
  })
);
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    process.env.APPIMAGE = "/path/EchoCraft.AppImage";
    setAutostartEnabled(true);
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    process.env.APPIMAGE = "/new/path/EchoCraft.AppImage";
    writeEntry(
      getDesktopFilePath(),
      '[Desktop Entry]\nType=Application\nExec="/old/path/EchoCraft.AppImage"\nX-GNOME-Autostart-enabled=false\n'
    );
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    process.env.APPIMAGE = "/installed/EchoCraft.AppImage";
    setAutostartEnabled(true);
    ...
    const contents = fs.readFileSync(getDesktopFilePath(), "utf8");
    assert.match(contents, /^Exec="\/installed\/EchoCraft\.AppImage" --hidden$/m);
  })
);
