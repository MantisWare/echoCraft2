...
const DEFAULT_OAUTH_PROTOCOL_BY_CHANNEL = {
  development: "echocraft-dev",
  staging: "echocraft-staging",
  production: "echocraft",
};
const BASE_WINDOWS_APP_ID = "com.mantisware.echocraft";
...
function configureChannelUserDataPath() {
  ...
  const isolatedPath = path.join(app.getPath("appData"), `EchoCraft-${APP_CHANNEL}`);
...
if (process.platform === "linux") {
  app.setDesktopName("echocraft.desktop");
}
...
// True source of truth for whether echocraft:// resolves on Linux — the same
...
if (process.platform === "darwin" && app.getName() !== "EchoCraft 2.0") {
  app.setName("EchoCraft 2.0");
}
...
function startAuthBridgeServer() {
  ...
  authBridgeServer = http.createServer(async (req, res) => {
    ...
    res.end(
      "<html><body><h3>EchoCraft sign-in complete.</h3><p>You can close this tab.</p></body></html>"
    );
  });
...
async function startApp() {
  ...
  const { copyLastTranscription } = require("./src/helpers/lastTranscription");
  trayManager.setCopyLastTranscription(() =>
    copyLastTranscription({
      getTranscriptions: (limit) => databaseManager.getTranscriptions(limit),
      writeClipboard: (text) => clipboardManager.writeClipboard(text),
    })
  );
